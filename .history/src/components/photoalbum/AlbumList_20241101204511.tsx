...
const AlbumList = () => {
  ...
  const { edit, setEdit, deleteId, setDeleteId, handleCheckboxChange, onHandleDelete, handleTouthDown } =
    useAlbumDelete();
  ...
  //   }
  // };
  const onClickImg = (url, id) => {
    onClickImgModal(url, id);
    handleTouthDown();
  };
          ...
          {albumListData?.map((item) => (
            ...
            >
              {item.photoImg && (
                <>
                  <Image
                    onClick={() => onClickImg(item.photoImg, item.id)}
                    src={item.photoImg}
                    alt=""
                    width={200}
                    height={200}
